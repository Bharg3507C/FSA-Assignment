...
function buildWhatsAppLink(phoneNumber, message) {
  const normalized = (phoneNumber || "12013899402").toString().trim();
  const digitsOnly = normalized.replace(/[^\d+]/g, "");
  const cleanNumber = digitsOnly.startsWith("+") ? digitsOnly : digitsOnly.replace(/^0/, "");
  const encodedText = encodeURIComponent(message);
  return `https://wa.me/${cleanNumber}?text=${encodedText}`;
}

function buildLeaveBotPrompt() {
  return [
    "Hello! I’m EFAR’s leave assistant.",
    "",
    "I’ll help you submit your leave request.",
    "",
    "Please reply with your leave details in this format:",
    "Full Name: <your full name>",
    "Leave Dates: <start date to end date>",
    "Reason: <reason for leave>",
    "MC / Supporting Details: <MC or notes>",
    "",
    "Validation rules:",
    "- Full name must be at least 2 letters.",
    "- Leave dates must include a date range.",
    "- Reason must be at least 10 characters.",
    "- MC / supporting details cannot be blank.",
    "",
    "If anything is missing or invalid, I will ask you to resend it."
  ].join("\n");
}

function validateLeaveReply(reply) {
  const normalized = reply || "";
  const fullNameMatch = normalized.match(/Full Name:\s*(.+)/i);
  const datesMatch = normalized.match(/Leave Dates:\s*(.+)/i);
  const reasonMatch = normalized.match(/Reason:\s*(.+)/i);
  const mcMatch = normalized.match(/MC\s*\/\s*Supporting Details:\s*(.+)/i);

  const fullName = fullNameMatch ? fullNameMatch[1].trim() : "";
  const leaveDates = datesMatch ? datesMatch[1].trim() : "";
  const reason = reasonMatch ? reasonMatch[1].trim() : "";
  const mcDetails = mcMatch ? mcMatch[1].trim() : "";

  if (!fullName || fullName.length < 2) {
    return { valid: false, error: "Full name must be at least 2 letters." };
  }
  if (!leaveDates || !/to/i.test(leaveDates) || !/\d/.test(leaveDates)) {
    return { valid: false, error: "Leave dates must include a date range such as 10 Jun 2026 to 12 Jun 2026." };
  }
  if (!reason || reason.length < 10) {
    return { valid: false, error: "Reason must be at least 10 characters." };
  }
  if (!mcDetails || mcDetails.length < 3) {
    return { valid: false, error: "MC / supporting details cannot be blank." };
  }

  return { valid: true, message: "Thank you, your leave request looks complete." };
}
  ...
  return (
    <div style={{ background:G.hero,border:`1px solid ${C.border}`,borderRadius:16,padding:"52px 48px",marginBottom:24,position:"relative",overflow:"hidden" }}>
      ...
      <div style={{ display:"flex",gap:12,flexWrap:"wrap",marginBottom:28 }}>
        <a href="https://wa.me/12013899402" target="_blank" rel="noreferrer"
...
/* ── Leave Management ── */
function LeaveTab() {
  const openWhatsAppFlow = (message) => {
    if (typeof window !== "undefined") {
      const waLink = buildWhatsAppLink("12013899402", message);
  ...
  const startLeaveRequest = () => {
    openWhatsAppFlow(buildLeaveBotPrompt());
  };

  const approveLeaveInWhatsApp = () => {
    openWhatsAppFlow(
      "Hello! I am reviewing this leave request.\n\nPlease approve it in this chat.\nIf it is not approved, I will send a rejection message with the reason.\n\nStatus: Pending approval"
    );
  };
    ...
    <>
      <div style={{ background:G.hero,border:`1px solid ${C.border}`,borderRadius:14,padding:"26px 32px",marginBottom:20,position:"relative",overflow:"hidden" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:14 }}>
          ...
          <div>
            <div style={{ fontSize:16,fontWeight:700,color:C.white,marginBottom:4 }}>Leave Management in WhatsApp</div>
            <div style={{ fontSize:12.5,color:C.muted,lineHeight:1.6 }}>
              The full leave process now happens inside WhatsApp. The website only opens the chat so users and staff can complete everything there.
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20 }}>
        <SCard title="📝 Start Leave Request" sub="Open WhatsApp and begin the leave flow there" g={G.teal}>
          <div style={{ color:C.sub,fontSize:13,lineHeight:1.7,marginBottom:14 }}>
            The bot sends the first message, asks for the required details, and validates each reply before moving forward.
          </div>
          <div style={{ background:C.cardDeep,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",marginBottom:12 }}>
            <div style={{ fontSize:12,fontWeight:700,color:C.white,marginBottom:8 }}>Validation rules</div>
            <ul style={{ margin:0,paddingLeft:16,color:C.sub,fontSize:12,lineHeight:1.7 }}>
              <li>Full name must be at least 2 letters.</li>
              <li>Leave dates must include a date range.</li>
              <li>Reason must be at least 10 characters.</li>
              <li>MC / supporting details cannot be blank.</li>
            </ul>
          </div>
          <button type="button" onClick={startLeaveRequest} style={{ width:"100%",padding:"11px",borderRadius:8,backgroundImage:G.pink,color:"#fff",fontWeight:700,fontSize:14,border:"none",cursor:"pointer" }}>Open WhatsApp Leave Chat</button>
        </SCard>

        <SCard title="✅ Staff Approval Flow" sub="Approve requests directly inside WhatsApp" g={G.pink}>
          <div style={{ color:C.sub,fontSize:13,lineHeight:1.7,marginBottom:14 }}>
            Staff can open WhatsApp and approve the leave request from the same chat. Once approved, the system can send the congratulatory recovery message there as well.
          </div>
          <button type="button" onClick={approveLeaveInWhatsApp} style={{ width:"100%",padding:"11px",borderRadius:8,backgroundImage:G.teal,color:"#fff",fontWeight:700,fontSize:14,border:"none",cursor:"pointer" }}>Open WhatsApp Approval Chat</button>
        </SCard>
      </div>

      <SCard title="📌 WhatsApp Flow" g={G.teal}>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12 }}>
          {[
            { step:"1",label:"Start Chat",desc:"User opens WhatsApp from the website.",col:C.teal },
            { step:"2",label:"Send Details",desc:"User shares full name, dates, reason, and MC in the chat.",col:C.blue },
            { step:"3",label:"Approve",desc:"Staff reviews and approves the request in WhatsApp.",col:C.orange },
            { step:"4",label:"Recovery Message",desc:"A congratulatory recovery message is sent back in the same WhatsApp thread.",col:C.green },
          ].map((s) => (
            <div key={s.step} style={{ background:C.cardDeep,borderRadius:10,padding:"16px",border:`1px solid ${C.border}`,textAlign:"center" }}>
              <div style={{ width:32,height:32,borderRadius:"50%",background:`${s.col}20`,border:`1px solid ${s.col}50`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:s.col,margin:"0 auto 10px" }}>{s.step}</div>
              <div style={{ fontSize:12.5,fontWeight:700,color:C.white,marginBottom:4 }}>{s.label}</div>
    ...
    <div style={{ display:"flex",height:"100vh",background:C.bg,fontFamily:"'Inter','Segoe UI',sans-serif",color:C.text,overflow:"hidden" }}>
      ...
      <div style={{ flex:1,display:"flex",flexDirection:"column",overflow:"hidden" }}>
        ...
        <div style={{ background:C.sidebar,borderBottom:`1px solid ${C.border}`,padding:"0 28px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0 }}>
          <input type="text" placeholder="🔍  Search..." style={{ flex:1,maxWidth:380,background:C.cardDeep,border:`1px solid ${C.border}`,borderRadius:8,padding:"7px 14px",color:C.text,fontSize:13,outline:"none" }} />
          <div style={{ display:"flex",alignItems:"center",gap:14 }}>
